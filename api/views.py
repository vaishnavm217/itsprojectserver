    ...
    for i in range(len(temp)):
        ...
        for j in range(len(temp2)):
            ...
            for j1 in range(len(temp5)):
                k = temp5[j1]["WID"]
                ser = YieldsSerializer(Yields.objects.filter(WID=k),many=True)
                pho = PhotosSerializer(Photos.objects.filter(Type="WID",WID__in=[obj.WID for obj in Photos.objects.filter(Type="WID") if obj.ID.WID==k]),many=True)
                vid = VideosSerializer(Videos.objects.filter(Type="WID",WID__in=[obj.WID for obj in Videos.objects.filter(Type="WID") if obj.ID.WID==k]),many=True)
                aud = AudiosSerializer(Audios.objects.filter(Type="WID",WID__in=[obj.WID for obj in Audios.objects.filter(Type="WID") if obj.ID.WID==k]),many=True)
                temp5[j1]["Yields"] = ser.data
                temp5[j1]["Photos"] = pho.data
                temp5[j1]["Audios"] = aud.data
                temp5[j1]["Videos"] = vid.data
            pho = PhotosSerializer(Photos.objects.filter(Type="FID",FID__in=[obj.FID for obj in Photos.objects.filter(Type="FID") if obj.ID.FID==k]),many=True)
            vid = VideosSerializer(Videos.objects.filter(Type="FID",FID__in=[obj.FID for obj in Videos.objects.filter(Type="FID") if obj.ID.FID==k]),many=True)
            aud = AudiosSerializer(Audios.objects.filter(Type="FID",FID__in=[obj.FID for obj in Audios.objects.filter(Type="FID") if obj.ID.FID==k]),many=True)
        ...
        videos=VideosSerializer(Videos.objects.filter(Type="HID",HID__in=[obj.HID for obj in Videos.objects.filter(Type="HID") if obj.ID.HID==temp[i]["HID"]]),many=True)
        data[i]["Videos"]=videos.data
        audios=AudiosSerializer(Audios.objects.filter(Type="HID",HID__in=[obj.HID for obj in Audios.objects.filter(Type="HID") if obj.ID.HID==temp[i]["HID"]]),many=True)
